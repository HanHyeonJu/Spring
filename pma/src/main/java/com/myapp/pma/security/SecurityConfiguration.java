...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		/**
		 * 메모리에 유저, 비밀번호, roles(허용범위 유저, 관리자 등) auth.inMemoryAuthentication()
		...
		auth.jdbcAuthentication()
			...
			.dataSource(dataSource).passwordEncoder(bCryptPasswordEncoder); // 암호화된 패스워드를 다시 디코딩해서 입력된 비번과 비교
	}

	/**
	 * @Bean // 패스워드 암호화 객체, 인증 시 password에 사용하기 위해 객체화 public PasswordEncoder
